...
namespace Folly.Services
{
    public class UserService : IUserService
    {
        ...
        public UserService(FollyDbContext dbContext, IHttpContextAccessor httpContextAccessor, ILanguageService languageService, IRoleService roleService)
        {
            DbContext = dbContext;
        ...
        public async Task<DTO.User> GetUserById(int id)
                    => await DbContext.Users.Include(x => x.UserRoles).Where(x => x.Id == id && x.Status == true).Select(x => MapUserToDTO(x)).FirstOrDefaultAsync();

        public async Task<DTO.User> GetUserByUsername(string userName)
            => await DbContext.Users.Where(x => x.UserName == userName && x.Status == true).Select(x => MapUserToDTO(x)).FirstOrDefaultAsync();
